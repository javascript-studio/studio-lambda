...
function fork_lambda(base_dir, lambda_path_tpl, name, environment, timeout,
  ...
  let idle_timer;
  proc.on('error', (err) => {
    log.error('Lambda process error', { name }, err);
    if (idle_timer) {
  ...
  proc.on('exit', (code, signal) => {
    if (code) {
      log.error('Lambda process exit', { name, code, signal });
    }
    if (idle_timer) {
      clearTimeout(idle_timer);
      idle_timer = null;
  ...
  return {
    invoke(event, context, callback) {
      ...
      timeout_timer = setTimeout(() => {
        log.warn('Lambda timeout', { name, ms_timeout });
        kill('SIGKILL');
      ...
      rpcs[++id] = (err, value) => {
        ...
        log.output('Lambda response', { name, id, response });
        callback(err, value);
      };
      const log_event = format_json(event);
      log.input('Lambda event', { name, id, event: log_event });
...
function kill_handler(entries, index, name) {
  return () => {
    log.terminate('Lambda terminated', { name });
...
function fork_pool(base_dir, lambda_path_tpl, config_file_tpl, name,
  ...
  return {
    invoke(event, context, callback) {
      ...
      if (!entry) {
        log.launch('Lambda launching', { name });
