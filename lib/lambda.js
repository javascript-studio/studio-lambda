...
function lambdaConfig(base_dir, config_file_tpl, name, default_env) {
  ...
  let config_file = replaceVars(config_file_tpl,
...
function forkLambda(base_dir, lambda_path_tpl, name, environment, timeout,
  ...
  proc.stdout.pipe(new ParseTransform({
  ...
  return {
    invoke(event, options, callback) {
      ...
      const context = {
        timeout,
        functionName: name,
        memoryLimitInMB: options.memory || DEFAULT_LAMBDA_MEMORY,
        awsRequestId: options.awsRequestId || String(requestId++)
      };
      entry.active = true;
...
exports.create = function (base_options = {}) {
  const lambda_path_tpl = base_options.lambda_path
    || 'functions/${LAMBDA_NAME}/index.js';
  const config_file_tpl = base_options.config_file
